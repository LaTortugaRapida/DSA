...
class DArray {
  ...
  resize(newCapacity, fill = 0) {
    if (!Number.isInteger(newCapacity) || newCapacity < 0) throw new TypeError("Capacity must be a positive integer!");
    let newArr = new Uint32Array(newCapacity);
    let small = Math.min(newCapacity, this.#size);
    if(newCapacity > this.#size) newArr.fill(fill);
   ...
   */
  pop_back() {
    if (this.empty()) throw new Error("The array is empty!");
  ...
  setValue(i, value) {
    if (this.empty()) throw new Error("The array is empty!");
    if (!Number.isInteger(i)) throw new TypeError("Index must be a positive integer!");
    if (i < 0 || i > this.#size - 1) throw new Error("Index is out of bound!");
  ...
  [Symbol.iterator]() {
    ...
    return {
      next: () => {
        if(this.empty()) return { value: undefined, done: true };
        if (i < this.#size) {
   ...
   */
  insert(pos, value) {
    if(!Number.isInteger(pos) || pos < 0) throw new TypeError("Position must be a positive integer");
    if(typeof value !== "number") throw new TypeError("Value must be of type number!");
    if(pos >= this.#size) this.resize(this.#capacity * this.CAP_EXPONENT);
    for(let i = pos + 1; i < this.#size; ++i) {
   ...
   */
  swap(i, j) {
    if (this.#size <= 1) throw new Error("No enough elements for swapping!");
    if (!Number.isInteger(i) || i < 0 || !Number.isInteger(j) || j < 0) throw new TypeError("Index must be a positive integer!");
    if (i < 0 || i > this.#size - 1 || j < 0 || j > this.#size - 1) throw new Error("Index is out of bound!");
    if (i === j) return;
  ...
  keys() {
    ...
    return {
      next: () => {
        if(i < this.#size) return {value: i++, done: false};
  ...
  entries() {
    ...
    return {
      next: () => {
        if(this.empty()) return { value: undefined, done: true };
        if(i < this.#size) return {value: [i, this.#arr[i++]], done: false};
        else return {value: undefined, done: true};
      }
    };
  }
  ...
  reduce(callback, initialValue) {
    if(typeof callback !== "function") throw new TypeError("Callback must be a function!");
    if(this.empty() || initialValue === undefined) throw new TypeError("Empty array or no initialValue!")
  ...
  some(callback, thisArg) {
    if(typeof callback !== "function") throw new TypeError("Callback must be a function!");
    if(this.empty()) throw new TypeError("Empty array!")
  ...
  find(callback, thisArg) {
    ...
    if(this.empty()) throw new TypeError("Empty array!")
    for(let i = 0; i < this.#size; ++i) {
      if (callback(thisArg, this.#arr[i], i, this.#arr)) return this.#arr[i];
    }
    return undefined;
  }
  ...
  findIndex(callback, thisArg) {
    ...
    if(this.empty()) throw new TypeError("Empty array!")
    for(let i = 0; i < this.#size; ++i) {
      if (callback(thisArg, this.#arr[i], i, this.#arr)) return i;
    }
    return -1;
  }
